/* =========================================================
   english.js — English learning games
   Learn mode: tap → hear the English word (always EN voice).
...
(function (global) {
  ...
  var UI = {
    title:    { en: 'Ali English — first words and letters',
                uz: 'Ali English — ilk so\'zlar va harflar',
                ru: 'Ali English — первые слова и буквы' },
    subtitle: { en: 'Let\'s learn English letters and first words! 🎉',
                uz: 'Ingliz harflari va ilk so\'zlarni o\'rganamiz! 🎉',
                ru: 'Учим английские буквы и первые слова! 🎉' },
    abcName:  { en: 'ABC — Letters', uz: 'ABC — Harflar', ru: 'ABC — Буквы' },
    abcDesc:  { en: 'The English alphabet', uz: 'Ingliz alifbosi', ru: 'Английский алфавит' },
  ...
  function openLetters() {
    ...
    $('learnTitle').textContent = '🔤 ABC';
    $('learnHint').textContent = L(UI.tapHint);
    var grid = $('learnGrid');
    grid.className = 'letter-grid';
    grid.innerHTML = '';
    Vocab.ALPHABET.forEach(function (a, i) {
      ...
      t.addEventListener('click', function () {
        Sound.unlock();
        popCard(t);
        Sound.say(a.l + '. ' + a.l + ' is for ' + a.w + '.', 'en');
      });
      grid.appendChild(t);
    });
    show('learn');
  ...
  function openWords(catId) {
    ...
    cat.words.forEach(function (w, i) {
      ...
      c.innerHTML = '<span class="we">' + w.e + '</span>' +
                    '<span class="wen">' + w.en + '</span>' +
                    '<span class="wtr">' + Vocab.trans(w) + '</span>';
  ...
  function startQuiz() {
    ...
    if (S.mode === 'letters') {
      S.list = shuffle(Vocab.ALPHABET.slice()).slice(0, QSET_SIZE);
    } else {
  ...
  function speakQ() {
    ...
    if (S.mode === 'letters') Sound.say('Find the letter ' + item.l + '.', 'en');
    else Sound.say('Where is the ' + item.en + '?', 'en');
  }
  ...
  function renderQ() {
    ...
    if (S.mode === 'letters') {
      ...
      others = shuffle(Vocab.ALPHABET.filter(function (a) { return a.l !== item.l; })).slice(0, 3);
      options = shuffle([item].concat(others));
    } else {
      $('qTitle').textContent = L(UI.find);
      $('bigWord').innerHTML = item.en + '<span class="tr">' + Vocab.trans(item) + '</span>';
  ...
  function answer(btn, opt, item) {
    ...
    setTimeout(function () {
      Sound.say(S.mode === 'letters' ? item.l : item.en, 'en');
    }, 500);
